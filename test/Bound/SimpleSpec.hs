...
import Bound.Simple (Bound(..), Scope, toScope, fromScope, abstract, abstract1, instantiate, instantiate1, Generically(..), closed)
...
data Exp a
  ...
  deriving (Functor, Foldable, Show, Eq, Generic1)
  deriving (Eq1, Show1) via (Generically Exp)
...
spec :: Spec
spec = describe "cooked" $
  it "should reduce correctly to normal form" $ do
